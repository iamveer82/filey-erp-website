...
export default function BuyPlan({ plan, label, primary }: { plan: 'cloud' | 'freedom'; label: string; primary?: boolean }) {
  ...
  return <form onSubmit={buy} className="buy-form">
    <label htmlFor={`buy-${plan}`}>Where should we send your licence?</label>
    <input
      id={`buy-${plan}`}
      ref={input}
      type="email"
    ...
    <p className="buy-hint">Sign in to Filey with this address and your plan is already there.</p>
  </form>
}
