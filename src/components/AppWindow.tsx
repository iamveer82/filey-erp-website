...
export function AppWindow({
  ...
  return (
    ...
    >
      {/* Title bar */}
      <div className="relative flex h-10 items-center justify-between border-b border-zinc-200 bg-white px-4">
        <div className="flex items-center gap-2" aria-hidden>
          <span className="h-2.5 w-2.5 rounded-full bg-rose-400" />
          <span className="h-2.5 w-2.5 rounded-full bg-amber-400" />
          <span className="h-2.5 w-2.5 rounded-full bg-emerald-400" />
        </div>
        <span className="pointer-events-none absolute left-1/2 -translate-x-1/2 font-mono text-[11px] text-zinc-400">
          {title}
        </span>
      ...
      <div className="flex min-h-0">
        {sidebar ? (
          <div className="shrink-0 border-r border-zinc-200 bg-white/60">{sidebar}</div>
        ) : null}
        <div className={cn('min-w-0 flex-1 bg-white p-4 lg:p-6', contentClassName)}>{children}</div>
      </div>
    </div>
  )
}
