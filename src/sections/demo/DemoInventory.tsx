...
function StatusChip({ status }: { status: StockStatus }) {
  return (
    <span
      className={cn(
        'inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] uppercase tracking-[0.1em]',
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="ml-auto text-[10.5px] tabular-nums text-zinc-400">
      ...
      <div className="mt-3 overflow-hidden rounded-xl border border-zinc-200">
        <Table>
          <TableHeader>
            <TableRow className="border-zinc-200 bg-white/70 hover:bg-white/70">
              {COLUMNS.map((col) => {
                ...
                return (
                  <TableHead key={col.key} className="h-9 px-3">
                    <button
                      ...
                      className={cn(
                        'flex items-center gap-1.5 text-[10px] uppercase tracking-[0.12em] transition-colors duration-200',
                        active ? 'text-amber-400' : 'text-zinc-400 hover:text-zinc-900',
                      )}
          ...
          <TableBody>
            {rows.map((r) => (
              <TableRow key={r.sku} className="border-zinc-200/60 hover:bg-amber-400/5">
                <TableCell className="px-3 py-2.5 text-[11.5px] tabular-nums text-zinc-400">{r.sku}</TableCell>
                <TableCell className="px-3 py-2.5 text-[13px] font-medium text-zinc-900">{r.item}</TableCell>
                <TableCell className="px-3 py-2.5 text-[12px] text-zinc-400">{r.category}</TableCell>
            ...
            {rows.length === 0 && (
              <TableRow className="border-zinc-200/60 hover:bg-transparent">
                <TableCell colSpan={5} className="px-3 py-10 text-center text-[11px] text-zinc-400">
